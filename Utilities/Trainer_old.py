...
class Trainer:
    ...
    def validation_error(self, epoch):
        error = self.l2_error(self.val_in, self.val_out, dims=[0, 1, 3])
        try:
    ...
    def compute_loss(self, method, losses, errors, weights, **kwargs):
        if (method is None) or (method == "lr_anneling") or (method == "my_algo"):
    ...
    def restart(self, weighting_dict, optim_dict, **kwargs):
