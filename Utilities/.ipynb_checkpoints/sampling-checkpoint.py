...
def _rad_fn(sample_fn, func, num_in, num_out, k, c, seed=1234, **kwargs):
    ...
    *sample, output = sample_fn(num_in, **kwargs)
    abs_value = torch_out(func)(*sample).abs().flatten()
    with torch.no_grad():
...
class Sampler:
    ...
    def sample(self, n, tensor=True, device='cpu', half=False):
...
### Sample from data
class data_sampler:
    def __init__(self, data, adapt=False, device="cuda"):
        n, m = data.shape
        if adapt:
            *inputs, outputs = np.hsplit(data, m)
            density = np.abs(outputs)/np.abs(outputs).sum()
        else:
            inputs = np.hsplit(data, m)
            density = (1 / n) * np.ones(n)

        self.inputs = inputs
        self.density = torch.Tensor(density.flatten()).to(device)
        self.device = device
    ...
    def sample(self, n, **kwargs):
        sample = self._Array_to_Tensor(self.inputs, self.device)
